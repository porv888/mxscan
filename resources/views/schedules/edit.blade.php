...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Modify automated scanning for {{ $schedule->domain->domain }}</p>
        </div>
        ...
        <form action="{{ route('schedules.update', $schedule) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="domain_id" id="domain_id" required
                    ...
                    @foreach($domains as $domain)
                        <option value="{{ $domain->id }}" {{ $schedule->domain_id == $domain->id ? 'selected' : '' }}>
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="dns_security" name="scan_type" value="dns_security" type="radio" 
                                   {{ $schedule->scan_type == 'dns_security' ? 'checked' : '' }}
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="blacklist" name="scan_type" value="blacklist" type="radio" 
                                   {{ $schedule->scan_type == 'blacklist' ? 'checked' : '' }}
                                   class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                        </div>
                        <div class="ml-3 text-sm">
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="both" name="scan_type" value="both" type="radio" 
                                   {{ $schedule->scan_type == 'both' ? 'checked' : '' }}
                                   class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                        </div>
                        <div class="ml-3 text-sm">
            ...
            <div>
                ...
                <select name="frequency" id="frequency" required
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="daily" {{ $schedule->frequency == 'daily' ? 'selected' : '' }}>Daily (2:00 AM)</option>
                    <option value="weekly" {{ $schedule->frequency == 'weekly' ? 'selected' : '' }}>Weekly (Monday 2:00 AM)</option>
                    <option value="monthly" {{ $schedule->frequency == 'monthly' ? 'selected' : '' }}>Monthly (1st of month 2:00 AM)</option>
                </select>
                @error('frequency')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-start">
                    ...
                    <div class="text-sm">
                        <p class="text-blue-800 font-medium">Current Schedule Information</p>
                        <ul class="text-blue-700 mt-1 space-y-1">
                            <li>• Status: <span class="font-medium">{{ ucfirst($schedule->status) }}</span></li>
                            @if($schedule->next_run_at)
                                <li>• Next run: <span class="font-medium">{{ $schedule->next_run_at->format('M j, Y g:i A') }}</span></li>
                            @endif
                            @if($schedule->last_run_at)
                                <li>• Last run: <span class="font-medium">{{ $schedule->last_run_at->diffForHumans() }}</span></li>
                            @endif
                        </ul>
                    </div>
                </div>
